...
public class ParticipantDAO implements IParticipantDAO, QueryConstants {
	...
	public static List<TrainingParticipantEnrollment> getParticipantEnrollemntList(int participantId,
			EntityManager manager) {

		TypedQuery<TrainingParticipantEnrollment> query = manager.createQuery(GET_PARTICIPANT_ENROLLMENT_LIST,
				TrainingParticipantEnrollment.class);
	...
	public List<TrainingProgram> getTrainingProgram(int participantId) {
		...
		List<TrainingParticipantEnrollment> trainingEnrollmentlist = ParticipantDAO
				.getParticipantEnrollemntList(participantId, manager);
		for (TrainingParticipantEnrollment program : trainingEnrollmentlist) {
			TrainingProgram getProgram = manager.find(TrainingProgram.class,
					program.getTrainingParticipantId().getTrainingCode());
	...
	public List<FeedbackMaster> getProvidedFeedback(int participantId) {
		...
		for (TrainingParticipantEnrollment program : trainingEnrollmentlist) {
			// System.out.println(program.getTrainingParticipantId().getParticipantId()+"
			// "+program.getTrainingParticipantId().getTrainingCode());
